...
0.3.0 is the current release candidate. PyPI still serves 0.2.3 until
this version is published. Nothing after 0.3.0 has been merged yet.

## [0.3.0] - unreleased

Release candidate. Package version is `0.3.0`. Not on PyPI yet.

### Changed
- Public street-view namespace is `uc.svi` / `urbancode[svi]`. `uc.streetview` stays a compatibility alias with a migration page
- `urbancode[svi]` is the local TCIS / color extra and no longer pulls ZenSVI or streetlevel
- `urbancode[download]` is the only extra that installs ZenSVI / streetlevel; `uc.svi.fetch` now hints that extra
- `urbancode[perception]` includes the TCIS runtime (`transformers`, `accelerate`, `safetensors`)
...
- `uc.svi.comfort()` is deprecated; it still returns a DataFrame and keeps `thermal_comfort` as a one-cycle alias of VATA
...
- Canonical pocket fixture is `examples/data/real/punggol`; `examples/data/punggol_pocket` is archived
...
- Lazy raster load raises `MissingExtraError` when `urbancode[imagery]` is missing instead of returning a path string
- `IndicatorResult.save` / `load` restore `AnalysisUnits.metadata` and reject unknown `on_duplicate` policies

### Added
- `uc.images.from_table` and `uc.svi.as_layer` accept a catalog path (JSON/CSV/parquet)
...
- `Layer.materialize()` loads a deferred payload
...
- Extras: `perception`, `research`, `spatial-stats` (the last has no public API yet), `download`
...
- `uc.climate.utci` (canonical); `uc.backends.available` / `info` / `require` / `status` / `explain`
...
- Extras: `vector`, `svi`, `viz`, `standard` (`streetview` is an install alias of `svi`; `gallery` is an alias of `viz`)
- Unified Phase 0 ADRs (UrbanCode × StreetRAG); frozen as a future interface; no StreetRAG runtime
- Ecosystem adapters: `uc.adapters` lazy re-exports; `uc.svi.fetch` (ZenSVI / unofficial Google opt-in); package table in `docs/source/reference/ecosystem.rst`
...
- Canonical Punggol City fixture under `examples/data/real/punggol`
