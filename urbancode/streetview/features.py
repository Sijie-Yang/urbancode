...
def color(df, folder_path=None, filename_column='Filename'):
    ...
    for idx, row in _tqdm(df.iterrows(), total=len(df), desc="Processing images"):
        ...
        try:
            ...
            feature_data.append({k: np.nan for k in COLOR_FEATURE_NAMES})
...
def segmentation(df, folder_path, filename_column='Filename'):
    ...
    stack = _torch_stack()
    torch = stack["torch"]
    SegformerImageProcessor = stack["SegformerImageProcessor"]
    SegformerForSemanticSegmentation = stack["SegformerForSemanticSegmentation"]
    from urbancode.streetview.contract import load_feature_models_manifest

    spec = load_feature_models_manifest()["segformer"]
    model_name = spec["model"]
    revision = spec["revision"]
    processor_name = spec.get("processor") or model_name
...
def object_detection(df, folder_path, filename_column='Filename'):
    ...
    FasterRCNN_ResNet50_FPN_Weights = stack["FasterRCNN_ResNet50_FPN_Weights"]
    # Initialize model
    ...
    results = []
    for _, row in _tqdm(df.iterrows(), total=len(df), desc="Detecting objects"):
        img_path = os.path.join(folder_path, row[filename_column])
...
def scene_recognition(df, folder_path=None, filename_column='Filename'):
    ...
    from urbancode.streetview.places365 import ensure_places365_weights, places365_labels

    stack = _torch_stack()
    torch = stack["torch"]
    models = stack["models"]
    transforms = stack["transforms"]
    scene_model_file = str(ensure_places365_weights())
    scene_label_file = str(places365_labels())
    ...
    for _, row in _tqdm(df.iterrows(), total=len(df), desc="Recognizing scenes"):
        img_path = os.path.join(folder_path, row[filename_column])
